...
export const ja: SiteContent = {
  ...
  home: {
    ...
    safety: {
      ...
      body:
        "MEGA POWER は、安全性を単独の訴求ではなく、ハード、警報、記録、保守性を一体化した運用設計として組み込みます。",
  ...
  platform: {
    ...
    capability: {
      ...
      explainer: {
        label: "6S + EDR とは",
        body:
          "ここでいう 6S は、MEGA POWER のアーキテクチャを構成する 6 つの中核システム層、すなわち BWS、HCS、PCS、BMS、EMS、OMS を指します。EDR はイベント記録と故障追跡を担い、6S+EDR の安全アーキテクチャを完成させます。",
        systems: [
    ...
    management: {
      ...
      body:
        "MEGA POWER は、イベント記録、戦略ディスパッチ、監視、警報、O&M 実行を一つのデジタル運用レイヤーに統合し、規模の異なる蓄電システムに対応します。",
    ...
    security: {
      ...
      body:
        "MEGA POWER は、小規模・中規模・大規模案件に応じた EMS ローカル配備を支援し、データ収集、伝送、保存、アクセス制御、サーバ保護、監査管理まで多層の保護設計を整えています。",
      deployment: [
        "小規模配備：1MWh 未満",
        "中規模配備：1MWh から 5MWh",
        "大規模配備：5MWh 超",
  ...
  safety: {
    ...
    hero: {
      ...
      subtitle:
        "MEGA POWER は、統合された安全ロジックと運用設計によって、不確実性を抑えた運用を支えます。",
      highlight: "追跡性",
    },
    intro: {
  ...
  services: {
    ...
    strategy: [
      { title: "ローカルサービスチームモデル", body: "MEGA POWER は、市場理解と現地対応を前提にしたサービス体制を構築します。" },
      { title: "異文化コミュニケーショントレーニング", body: "国際協業の品質と信頼形成を支える要素です。" },
      { title: "継続的技術支援と更新", body: "MEGA POWER は、導入後も継続する技術支援と更新を長期運用の前提として重視します。" },
  ...
  about: {
    ...
    intro: {
      ...
      body:
        "MEGA POWER は、統合エネルギー設計、デジタル運用能力、サービス準備を組み合わせて、より完成度の高いソリューションを提供します。",
    },
    pillars: [
